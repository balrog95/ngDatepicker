		...
		return {
			...
			link: function (scope, element, attrs, ngModel) {
				...
				var generateCalendar = function (date) {
					...
					if (firstWeekDay > 1) {
						n -= firstWeekDay - 1;
					}if(firstWeekDay==0)n =-5;//fixed so that it can now handle Sunday as being the first day of the month
				...
				scope.initialDate = function(date){ //same as selectDate but without the event or closing the calendar. For setting date when page is loaded.
					var selectedDate = moment((date.day) + '.' + date.month + '.' + date.year, 'DD.MM.YYYY');
					ngModel.$setViewValue(selectedDate.format(scope.format));
					scope.viewValue = selectedDate.format(scope.viewFormat);
				}
				...
				var initDate = new Date(); //sets date to current day when you first load the page
				scope.initialDate({day:initDate.getDate(), month:initDate.getMonth()+1, year:initDate.getFullYear()});
			},
			template:
